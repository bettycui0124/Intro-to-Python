...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([c for c in centroids], key = lambda c: distance(location, c))
    # END Question 3
...
def best_predictor(user, restaurants, feature_fns):
    ...
    pred_r_square = {pred: r_square for pred, r_square in predictors}
...
def rate_all(user, restaurants, feature_fns):
    ...
    dict1 = {name: rating for name, rating in zip(rest_names, pred_rest_ratings)}
    user_rest_ratings = [user_rating(user, restaurant) for restaurant in reviewed_names]
    dict2 = {name: rating for name, rating in zip(reviewed_names, user_rest_ratings)}
    dict1.update(dict2)

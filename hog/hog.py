...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        gain = (score0 if who == 0 else score1) - previous_score
        if gain > previous_high:
            message = str(gain) + ' point'
            if gain > 1:
                message += 's'
            message += '! That\'s the biggest gain yet for Player ' + str(who)
            print(message)
        return announce_highest(who, max(gain, previous_high), (score0 if who == 0 else score1))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    return max([i for i in range(1, 11)], key = lambda x: make_averaged(roll_dice, num_samples)(x, dice))
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score + x, opponent_score) and score + x < opponent_score:
        return 0
    if not is_swap(score + x, opponent_score) and x >= margin:
        return 0
    return num_rolls
    # END PROBLEM 11

...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        bn = [replace_leaf(b, old, new) for b in branches(t)]
        return tree(label(t), bn)
...
def balanced(m):
    ...
    """
    def torque(t):
        return label(t) * total_weight(end(t))
    torque_balanced = torque(sides(m)[0]) == torque(sides(m)[1])
    if False in [balanced(end(i)) for i in sides(m) if not is_weight(end(i))]:
        return False
    if torque_balanced is not True:
        return False
    return True
#######
# OOP #
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while balance < amount:
            balance = (1 + self.interest) * balance
            i += 1
        return i
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(a) == str:
        return a
    def join(amount, guess):
        if guess == new_password:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, guess)
    return join
